        ...
        <ul className="flex flex-col gap-1.5">
          {events.map((ev) => (
            <li
              ...
              className="flex items-center gap-2 rounded-lg border border-gridwars-border/40 bg-gridwars-bg/40 px-2.5 py-1.5 text-xs animate-fade-in"
            >
              <span
                className="h-2.5 w-2.5 shrink-0 rounded-sm"
                style={{ backgroundColor: ev.tile.color ?? "#555" }}
              />
              <span className="truncate font-medium text-gridwars-text">
                {ev.tile.ownerName}
              </span>
              {ev.tile.wasSteal && (
                <span className="ml-auto shrink-0 text-[10px] text-gridwars-accent3">
                  steal
                </span>
              )}
            </li>
